...
class MDFEM(nn.Module):
    def __init__(self, dim, wavelet='haar', initialize=True, use_ca=False, use_sa=False):
        ...
        self.conv_edge = nn.Conv2d(dim, dim // 3, 1)
        self.edge=MdDetailPerception(dim // 3 )  
        self.conv_edge2 = nn.Conv2d(dim//3, dim, 1)
...
class SceneAndTexture(nn.Module):
    def __init__(self, channels):
        super(SceneAndTexture, self).__init__()
...
class AttentionAndFrequency(nn.Module):
    def __init__(self,
                 dim=256,
                 num_heads=16,
                 qkv_bias=False,
...
class Block(nn.Module):
    def __init__(self, dim=256, num_heads=16,  mlp_ratio=4., qkv_bias=False, drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = AttentionAndFrequency(dim, num_heads=num_heads, qkv_bias=qkv_bias, window_size=window_size)
...
class MLCM(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.fusion_conv = SceneAndTexture(out_channels)
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.MLCMS = nn.ModuleList([            
            MLCM(decode_channels * 3, decode_channels),
            MLCM(decode_channels * 3, decode_channels * 2),
            MLCM(decode_channels * 6, decode_channels * 4)
        ])
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        ...
        mlcms1 = self.MLCMS[0](res1, x2_1)
        mlcms1F = F.interpolate(mlcms1, scale_factor=0.5, mode="bilinear", align_corners=True)
       

        mlcms2 = self.MLCMS[1](res2, mlcms1F)
        mlcms2F = F.interpolate(mlcms2, scale_factor=0.5, mode="bilinear", align_corners=True)
       
        mlcms3 = self.MLCMS[2](res3, mlcms2F)

        mlcms1 = torch.permute(mlcms1, (0,2,3,1))
        mlcms2 = torch.permute(mlcms2, (0,2,3,1))
        mlcms3 = torch.permute(mlcms3, (0,2,3,1))
...
class DGMDC(nn.Module):
    def __init__(self,
        ...
        pretrained_cfg['file']=r'/ConvNext_timm_pretrain.bin'
        ...
        self.down = nn.Conv2d(768, 768 // 2, kernel_size=1, stride=1)
