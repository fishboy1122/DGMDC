...
class Lossing(nn.Module):
    def __init__(self, ignore_index=255):
        super().__init__()
        self.loss = nn.CrossEntropyLoss(weight=None, size_average=None, ignore_index=-28762, reduce=None, reduction='mean', label_smoothing=0.0)
    
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            logit_main, logit_aux = logits
            loss = self.loss(logit_main, labels) + 0.4 * self.loss(logit_aux, labels)
        else:
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            ...
            if isinstance(logit_aux, list):
                ...
                loss_aux = self.main_loss(h2, labels) + self.main_loss(h3, labels) + self.main_loss(h4, labels)
                # loss = self.main_loss(logit_main, labels) + 0.4 * self.aux_loss(logit_aux, labels)
